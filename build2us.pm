...
This API allows a channel/OTA (travel booking website) to integrate with
myallocator by implementing a public-facing API receiver in their environment.
...
Please see the example of format below (or see a full booking examples
[here](https://github.com/MyAllocator/bookingsamples)). If including payment data,
insert the Payments Array inside the Booking hash (see Minimal Booking Example
above). The only required fields are CardCode and CardNumber. The remaining fields
aren't required, but encouraged.
